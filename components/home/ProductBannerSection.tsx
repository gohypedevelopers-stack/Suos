...
export function ProductBannerSection({
  ...
  const desktopImg = banner.desktopImageUrl || "/images/products/product15.png"
  const mobileImg = banner.mobileImageUrl || desktopImg
  const link = banner.ctaLink

  const content = (
    <section className="group/product-banner relative w-full aspect-[16/7] sm:aspect-[1440/449] min-h-[150px] sm:min-h-0 overflow-hidden bg-black">
      ...
      {banner.mobileImageUrl ? (
        <>
          <div className="relative block sm:hidden w-full h-full">
            <Image
              src={mobileImg}
              alt={banner.title || "SUOS denim editorial banner"}
              fill
              sizes="100vw"
